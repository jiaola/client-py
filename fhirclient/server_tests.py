...
class TestServer(unittest.TestCase):
    ...
    def testInvalidCapabilityStatement(self):
        ...
        try:
            ...
            self.assertEqual("operation.1:", str(e.errors[2].errors[1])[:12])
            self.assertEqual("definition:", str(e.errors[2].errors[1].errors[0])[:11])
            self.assertEqual("Wrong type <class 'dict'>", str(e.errors[2].errors[1].errors[0].errors[0])[:25])
            self.assertEqual("security:", str(e.errors[2].errors[0])[:9])
            self.assertEqual("service.0:", str(e.errors[2].errors[0].errors[0])[:10])
            self.assertEqual("coding.0:", str(e.errors[2].errors[0].errors[0].errors[0])[:9])
            self.assertEqual("Superfluous entry \"systems\"", str(e.errors[2].errors[0].errors[0].errors[0].errors[0])[:27])
